...
export const createCartHeadless = (data: CustomFieldItems) =>
  B3Request.post(
    `${BigCommerceStorefrontAPIBaseURL}/api/storefront/carts`,
    RequestType.BCRest,
    data,
  ).then(rejectResponsesWithDetail);

export const getCartInfo = () =>
  B3Request.get(`${BigCommerceStorefrontAPIBaseURL}/api/storefront/carts`, RequestType.BCRest).then(
...
export const addProductToCart = (data: CustomFieldItems, cartId: string) =>
  B3Request.post(
    `${BigCommerceStorefrontAPIBaseURL}/api/storefront/carts/${cartId}/items`,
    RequestType.BCRest,
    data,
  ).then(rejectResponsesWithDetail);
...
export const getCartInfoWithOptions = () =>
  B3Request.get(
    `${BigCommerceStorefrontAPIBaseURL}/api/storefront/carts?include=lineItems.digitalItems.options,lineItems.physicalItems.options`,
    RequestType.BCRest,
  ).then(rejectResponsesWithDetail);

export const deleteCart = (cartId: string) =>
  B3Request.delete(
    `${BigCommerceStorefrontAPIBaseURL}/api/storefront/carts/${cartId}`,
    RequestType.BCRest,
  ).then(rejectResponsesWithDetail);
