...
const usePurchasableQuote = (openQuickView: boolean) => {
  ...
  const isOOStockPurchaseQuantity = useCallback(
    (qty: number, productPurchasable: ProductInfoProps): boolean => {
      ...
      if (featureFlags['B2B-3318.move_stock_and_backorder_validation_to_backend']) {
        ...
        // B2B-3679: Default behavior when feature flag is true, but API is not returning required backordering data
        return inventoryTracking && qty > inventoryLevel;
      }
  ...
  }, [openQuickView, isEnableProduct, isOOStockPurchaseQuantity]);
