...
export const initB2BInfo = (
  accountSettings: any,
  contactInformation: Array<Partial<Fields>>,
  accountB2BFormFields: Array<Partial<Fields>>,
  additionalInformation: Array<Partial<Fields>>,
) => {
  ...
  additionalInformation.forEach((item: Partial<Fields>) => {
    ...
    if (formFields) {
      infoItem.default = formFields.value;
    }
  });
...
export const b2bSubmitDataProcessing = (
  ...
  accountSettings: any,
  decryptionFields: Array<Partial<Fields>>,
  extraFields: Array<Partial<Fields>>,
) => {
    ...
    if (flag) {
      extraFields.forEach((field: Partial<Fields>) => {
        if (field.fieldId === key && param.formFields) {
          ...
          if (!accountSettings?.formFields?.length && name && Boolean(data[name])) {
            pristine = false;
          }
        }
      });
