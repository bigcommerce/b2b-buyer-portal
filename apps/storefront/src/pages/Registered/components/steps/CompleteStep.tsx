...
export default function CompleteStep(props: CompleteStepProps) {
  ...
  const createBCCustomer = (data: CustomFieldItems) => {
    const payload = buildBCUserPayload({
      list,
      additionalInfo,
      addressBasicList,
    ...
    return createBCCompanyUser({ storeHash, ...payload }, captchaKey).then((res) => ({
      customerId: res.customerCreate.customer.id,
      customerEmail: res.customerCreate.customer.email,
    }));
  };

  const createB2BCompany = async (
  ...
  const uploadCompanyAttachments = async (
    attachmentsList: RegisterFields[],
  ): Promise<CustomFieldItems[] | undefined> => {
    try {
      return await uploadAttachmentsToFileList(attachmentsList, uploadB2BFile, () =>
  ...
  const sendNewsletterSubscriptionIfOptedIn = async () => {
    const payload = getSubscribersPayload(list, enterEmail);
    if (!payload) return;
    try {
      await sendSubscribersState(payload);
    } catch (err: unknown) {
    ...
    handleSubmit(async ({ password, confirmPassword }: CustomFieldItems) => {
      ...
      if (!isCaptchaMissing) {
        try {
          ...
          if (accountType === '2') {
            await createBCCustomer(passwordData);
          } else {
            const attachmentsList = companyInformation.filter((item) => item.fieldType === 'files');
            const fileList = await uploadCompanyAttachments(attachmentsList ?? []);
            const { customerId, customerEmail } = await createBCCustomer(passwordData);
            const accountInfo = await createB2BCompany(
              customerId,
              customerEmail,
              fileList ?? undefined,
            );
            const companyStatus = accountInfo?.companyCreate?.company?.companyStatus ?? '';
            isAutoApproval = Number(companyStatus) === 1;
          }
          ...
          saveRegisterPassword(passwordData);
          await sendNewsletterSubscriptionIfOptedIn();
          handleNext(password);
        } catch (err: unknown) {
          setErrorMessage(err instanceof Error ? err.message : String(err));
        } finally {
