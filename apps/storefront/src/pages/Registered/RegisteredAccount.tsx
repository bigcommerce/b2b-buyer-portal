  ...
  const handleAccountToDetail = async (event: MouseEvent) => {
    handleSubmit(async (data: CustomFieldItems) => {
      ...
      try {
        ...
        if (accountType === '1') {
          const extraCompanyUserInformation = newContactInfo.filter((item: RegisterFields) =>
            Boolean(item.custom),
          );
      ...
      let newAdditionalInformation: RegisterFields[] = [];

      if (additionalInfo) {
        newAdditionalInformation = (additionalInfo as RegisterFields[]).map(
          (item: RegisterFields) => {
