...
const approvedB2BCompany = buildCompanyStateWith({
  ...
  customer: { userType: UserTypes.B2C, role: CustomerRole.B2C },
});
...
it('adds the skus and the quantities to the cart when clicking on the -add to cart- button', async () => {
  ...
  const [skuInput, missingSkuInput] = screen.getAllByRole('textbox', { name: 'SKU#' });
  const [qtyInput, missingQtyInput] = screen.getAllByRole('spinbutton', { name: 'Qty' });

  await userEvent.type(skuInput, 'S-123');
  await userEvent.type(qtyInput, '2');

  await userEvent.type(missingSkuInput, 'S-456');
  await userEvent.type(missingQtyInput, '3');
  ...
  expect(missingSkuInput).toHaveValue('S-456');
  expect(missingQtyInput).toHaveValue(3);
});
