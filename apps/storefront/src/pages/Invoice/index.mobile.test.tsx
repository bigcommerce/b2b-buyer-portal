...
const companyStateWithPurchasePermissions = buildCompanyStateWith({
  ...
  permissions: [
    ...
    { code: 'pay_invoice', permissionLevel: 1 },
...
it('renders invoice information in the table', async () => {
  ...
  expect(screen.getByRole('heading', { name: '4344' })).toBeInTheDocument();
  expect(screen.getByText('1234')).toBeInTheDocument();
  expect(screen.getByText('13 March 2025')).toBeInTheDocument();
  expect(screen.getByText('13 October 2025')).toBeInTheDocument();
  expect(screen.getByText('$922.00')).toBeInTheDocument();
  ...
  expect(menuItems[0]).toHaveTextContent('View invoice');
  expect(menuItems[1]).toHaveTextContent('View order');
  expect(menuItems[2]).toHaveTextContent('View payment history');
  expect(menuItems[3]).toHaveTextContent('Pay');
  expect(menuItems[4]).toHaveTextContent('Print');
  expect(menuItems[5]).toHaveTextContent('Download');
});
        ...
        buildInvoicesResponseWith({
          data: {
            invoices: {
              edges: [
                buildInvoiceWith({
                  node: { id: '3344', invoiceNumber: '3322', orderNumber: '4444' },
                }),
              ],
            },
          },
