...
const buildCustomerOrderStatusesWith = builder<CustomerOrderStatues>(() => ({
  data: {
    bcOrderStatuses: [
      buildOrderStatusWith({ systemLabel: 'Pending' }),
      buildOrderStatusWith({ systemLabel: 'Shipped' }),
      buildOrderStatusWith({ systemLabel: 'Cancelled' }),
    ],
  },
}));
    ...
    it('displays all the information associated with an order', async () => {
      const order66996 = buildCustomerOrderNodeWith({
        node: {
          ...
          totalIncTax: 100,
          status: 'Pending',
          createdAt: getUnixTime(new Date(2025, 2, 13)),
        },
      });
